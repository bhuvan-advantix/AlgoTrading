...
mongoose.connect(mongoDbUri, {
    ...
    .then(async () => {
        ...
        try {
            // Drop email index if it exists to allow duplicates/nulls
...
const UserSchema = new mongoose.Schema({
    ...
    email: { type: String, default: null }, // Optional & Non-unique
...
// --- Kite helper: create Kite client per-user using the stored access token ---
function makeKiteClient(accessToken, apiKey) {
    if (!accessToken) throw new Error('Access token is required to create Kite client');
    const kite = new KiteConnect({ api_key: apiKey || KITE_API_KEY });
