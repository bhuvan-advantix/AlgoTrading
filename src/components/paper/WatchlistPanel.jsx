// src/components/paper/WatchlistPanel.jsx
import React, { useState } from 'react';

export default function WatchlistPanel({ watchlist = [], quotes = {}, onSelect, refresh }) {
  const [input, setInput] = useState('');
  function add() {
    const s = input.trim().toUpperCase();
    if (!s) return;
    fetch(`/api/quote/${encodeURIComponent(s)}`).then(r=>r.json()).then(j=>{
